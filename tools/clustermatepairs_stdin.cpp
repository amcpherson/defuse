...
void OutputClusterMember(ostream& out, int clusterID, int clusterEnd, const CompactAlignment& alignment, const NameIndex& referenceNames, const NameIndex& fragmentNames)
{
	out << clusterID << "\t";
	out << clusterEnd << "\t";
	out << fragmentNames.Get(alignment.readID.fragmentIndex) << "\t";
	out << alignment.readID.readEnd << "\t";
	out << referenceNames.Get(alignment.refStrand.referenceIndex) << "\t";
...
int main(int argc, char* argv[])
{
	...
	while (getline(cin, line))
	{
		...
		qnameFields.push_back("");

		// Retreive fragment name and read end
		int fragmentIndex = fragmentNames.Index(qnameFields[0]);
		...
		alignment.readID.readEnd = readEnd;
		alignment.readID.fragmentIndex = fragmentIndex;
		alignment.refStrand.referenceIndex = referenceNames.Index(rname);
		alignment.refStrand.strand = strand;
		alignment.region.start = start;
		alignment.region.end = end;

		// Reads must be sorted by qname
		if (alignments.size() > 0 && alignments.front().readID.fragmentIndex > alignment.readID.fragmentIndex)
		{
			cerr << "Error: input alignments must be sorted by read" << endl;
			cerr << line << endl;
			exit(1);
		}
	...
	for (RefBinPairMapConstIter refBinIter = binPairs.begin(); refBinIter != binPairs.end(); refBinIter++)
	{
		...
		const string& refName1 = referenceNames.Get(refBin1.referenceIndex);
		const string& refName2 = referenceNames.Get(refBin2.referenceIndex);
		...
		for (int clusterIndex = 0; clusterIndex < matePairClusters.size(); clusterIndex++)
		{
			...
			for (IntegerVecConstIter elementIter = cluster.begin(); elementIter != cluster.end(); elementIter++)
			{
				...
				OutputClusterMember(clustersFile, clusterID, 0, alignment1, referenceNames, fragmentNames);
				OutputClusterMember(clustersFile, clusterID, 1, alignment2, referenceNames, fragmentNames);
			}
