...
#ifndef SPLITFUSION_H_
#define SPLITFUSION_H_
...
{
public:
	SplitFusion(const AlignmentIndex& discordant, const AlignmentIndex& anchored, const ReadIndex& reads, const FastaIndex& reference,
	            const ExonRegions& exonRegions, double fragmentLengthMean, double fragmentLengthStdDev, int maxReadLength, int minReadLength,
				ostream& logFile);
	
	bool Calculate(const LocationVec& alignPair, IntegerVec& breakPos, string& sequence, int& splitReadCount, double& splitPosAvg, double& splitMinAvg);
